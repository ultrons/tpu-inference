...
def get_cost_estimate(
  ...
  Args:
    ...
    cost_mult: Multiplier applied to the final flops and bytes_accessed
      (moe_sc_cost_estimate_mult). 1.0 (default) = real cost; >1.0 over-states
      the SC cost so XLA's layer scheduler biases toward overlapping the
      data-independent FSDP weight all-gather. Pure scheduling metadata ->
      numerics unchanged. Applied AFTER any override.
  ...
  # moe_sc_cost_estimate_mult: scale the (possibly overridden) cost so the XLA
  # layer scheduler can be biased toward overlapping the FSDP weight all-gather.
  if cost_mult != 1.0:
    flops = int(flops * cost_mult)
    bytes_accessed = int(bytes_accessed * cost_mult)
...
def main_kernel(
  ...
  def inner_kernel():
    ...
    # LEVER C3: VMEM scratch for the compacted run-end (src_row, dst) pairs (col-independent;
    # refilled once per row-tile). Tiny: num_simd_lanes int32 each.
    run_src_ref = jax.empty_ref(
        jax.core.ShapedArray((num_simd_lanes,), jnp.int32), memory_space=pltpu.VMEM)
    run_dst_ref = jax.empty_ref(
        jax.core.ShapedArray((num_simd_lanes,), jnp.int32), memory_space=pltpu.VMEM)
    ...
    def row_loop(row_block_id):
      ...
      # LEVER C3 compaction: build the run-end (src_row, dst) pairs as FULL-width (num_simd_lanes,)
      # vectors via jnp.where (the SC only supports full-width VMEM stores, not (1,) dynamic-slot
      # stores), then store once. A "run" = a maximal block of same-dst valid rows; its accumulated
      # sum lives in out_vmem_ref[run_end_row]. run_*_vec[k] = the k-th run-end's (src_row, dst);
      # slots >= n_runs are padding. We then write each run EXACTLY ONCE (n_runs writes, no pl.when).
      last_valid_local = jnp.minimum(
          num_simd_lanes,
          num_rows_current_row_partition - row_block_id * num_simd_lanes) - 1
      _iota = jax.lax.broadcasted_iota(jnp.int32, (num_simd_lanes,), 0)
      run_src_vec = jnp.zeros((num_simd_lanes,), jnp.int32)
      run_dst_vec = jnp.zeros((num_simd_lanes,), jnp.int32)
      n_runs = jnp.int32(0)
      for i in range(num_simd_lanes):
        valid_i = i <= last_valid_local
        if i == num_simd_lanes - 1:
          is_run_end = valid_i
        else:
          # i < last_valid => i+1 is valid (valid-first packing); a run ends when dst changes.
          is_run_end = jnp.logical_and(valid_i, dst_indices[i] != dst_indices[i + 1])
        # Place (i, dst[i]) into compaction slot n_runs iff row i is a run-end; advance only then.
        put = jnp.logical_and(_iota == n_runs, is_run_end)   # (num_simd_lanes,) 1-D bool
        run_src_vec = jnp.where(put, i, run_src_vec)
        run_dst_vec = jnp.where(put, dst_indices[i], run_dst_vec)
        n_runs = n_runs + jnp.where(is_run_end, 1, 0)
      run_src_ref[...] = run_src_vec
      run_dst_ref[...] = run_dst_vec
      ...
      def dma_write_loop(col_vmem_start, carry):
        ...
        for col_compute_offset in range(0, num_lanes, num_simd_lanes):
          ...
          for row_src in range(num_simd_lanes):
            ...
            if row_src == 0:
              # INDEPENDENT TILES: row 0 always starts a fresh run -> NO cross-tile
              # carry. Valid when runs never span a tile boundary (true at EP4:
              # len-2 runs, tile-aligned; general case wants the preprocess to
              # align runs to tiles). Dropping the prev_iter_last read/write +
              # carry[0] removes the tile->tile DATA dependency, so the compiler
              # can pipeline tile N+1's gather under tile N's (serial) reduce.
        ...
        # LEVER C3: write each run EXACTLY ONCE (n_runs data-dependent writes, no pl.when). The
        # compacted run_src_ref/run_dst_ref hold (run-end src_row, dst); out_vmem_ref[src_row] holds
        # that run's accumulated sum.
        # Load the compacted run-ends as full (num_simd_lanes,) vectors — the SC int32 Get requires the
        # full width (a (1,) dynamic-index read does NOT lower: "Get only supports ... (16,), got (1,)").
        # Extract element j via a select-reduce (vector ops, lowers cleanly).
        run_src_full = run_src_ref[...]
        run_dst_full = run_dst_ref[...]
        @pl.loop(0, n_runs)
        def _write(j):
          sel = _iota == j
          s = jnp.sum(jnp.where(sel, run_src_full, 0))
          d = jnp.sum(jnp.where(sel, run_dst_full, 0))
          pltpu.make_async_copy(
              out_vmem_ref.at[s, pl.ds(col_vmem_start, num_lanes)],
              out_32b_hbm_ref.at[d, pl.ds(col_hbm_start, num_lanes)],
              send_sem,
          ).start()

        return carry

      # LEVER C3: wait for exactly the writes we issued (n_runs per col-tile, matches the issue count).
...
def ragged_gather_reduce(
  ...
  Args:
    ...
    flops_override: If > 0, override the auto-computed flop count. -1 = auto.
    bytes_accessed_override: If > 0, override the auto-computed bytes_accessed.
      -1 = auto.
    cost_mult: Multiplier on the kernel cost_estimate (moe_sc_cost_estimate_mult).
      1.0 (default) = real cost; >1.0 over-states the SC cost so XLA's layer
      scheduler is biased toward overlapping the FSDP weight all-gather. Pure
      scheduling metadata -> numerics unchanged.
  ...
  out = pl.kernel(  # pytype: disable=wrong-keyword-args
      ...
      # Cost model so XLA's layer scheduler sees this SC gather+reduce as compute and can overlap the
      # FSDP weight all-gather against it. Upstream get_cost_estimate computes the real flops (gather
      # multiply + reduce adds) + HBM bytes (+ optional overrides); cost_mult (moe_sc_cost_estimate_mult)
      # scales it for the scheduler sweep.
